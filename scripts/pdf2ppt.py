...
"""notebookLM2PPT — single-file script

Convert NotebookLM PDF exports to PowerPoint with watermark removal and vector graphics.
Uses a PDF → SVG → EMF → PPTX pipeline to preserve vector quality.

Usage:
    uv run pdf2ppt.py input.pdf [output.pptx] [options]

System dependencies (required, not installable via pip/uv):
    Both `pdf2svg` and `inkscape` must be available on PATH.
    ...
    Windows:    choco install pdf2svg inkscape
                (or download Inkscape from https://inkscape.org)

    The AI harness should install these before invoking this script if the
    execution environment allows it.
...
def main():
    ...
    parser.add_argument('--check-deps', action='store_true',
                        help='Check system dependencies and exit. Returns JSON: {"ok": bool, "pdf2svg": bool, "inkscape": bool}')
    ...
    # --check-deps: validate system dependencies and exit with JSON result
    if args.check_deps:
        has_pdf2svg = check_dependency(args.pdf2svg_path, 'pdf2svg')
        has_inkscape = check_dependency(args.inkscape_path, 'inkscape')
        result = {"ok": has_pdf2svg and has_inkscape, "pdf2svg": has_pdf2svg, "inkscape": has_inkscape}
        print(json.dumps(result))
        sys.exit(0 if result["ok"] else ERR_DEPENDENCY_MISSING)
    if not args.input:
